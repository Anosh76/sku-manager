...
app.get('/skus', verifyToken, async (req, res) => {
  const skus = await SKU.find().sort({ timestamp: -1 });
  res.json(skus);
});

app.post('/skus', verifyToken, async (req, res) => {
  try {
    const { sku } = req.body;
...
app.delete('/skus/:id', verifyToken, async (req, res) => {
  await SKU.findByIdAndDelete(req.params.id);
  res.json({ message: 'Deleted' });
});

app.post('/skus/import', verifyToken, async (req, res) => {
  const { skus } = req.body;
